...
- 3afe738: Nested lists exported to Markdown now keep their hierarchy when the file is read back by other Markdown tools.
- 3afe738: Fix a denial-of-service risk where crafted block or inline Markdown attributes could consume excessive CPU and block the browser or server event loop.
- 3afe738: Fix freezes in framework-based node views on iOS and Android
- 3afe738: Prevent untrusted HTML attributes from changing an object's prototype when merged with `mergeAttributes`.
- Updated dependencies [3afe738]
...
- 3afe738: Pasting a YouTube iframe without a `src` attribute no longer crashes the editor. The embed is kept without a source.
- Updated dependencies [3afe738]
- Updated dependencies [3afe738]
- Updated dependencies [3afe738]
- Updated dependencies [3afe738]
...
- 3afe738: StarterKit no longer installs the deprecated `@tiptap/extension-dropcursor`, `@tiptap/extension-gapcursor`, `@tiptap/extension-list-item`, and `@tiptap/extension-list-keymap` packages. We will not publish further updates to these packages. StarterKit still includes their extensions, so you do not need to change your configuration.
...
- 3afe738: Fix Markdown serialization of whitespace-only marked text.
- Updated dependencies [3afe738]
- Updated dependencies [3afe738]
- Updated dependencies [3afe738]
- Updated dependencies [3afe738]
...
- 3afe738: Align `selected` with ProseMirror node selections by default, expose text selections through `selectionInside`, and keep `selectedOnTextSelection` compatible.

#### Patch Changes

- 3afe738: Reduce overhead in React node views on documents with many nodes.
- Updated dependencies [3afe738]
- Updated dependencies [3afe738]
- Updated dependencies [3afe738]
- Updated dependencies [3afe738]
...
- 3afe738: Bump `prosemirror-view` to `^1.42.3`, which fixes an XSS vulnerability where pasting crafted HTML could run arbitrary JavaScript (GHSA-c8x8-7fp4-3x9w).

### @tiptap/vue

#### Major Changes

- 3afe738: Vue 2 support is removed. `@tiptap/vue-3` is now `@tiptap/vue`, and `@tiptap/extension-drag-handle-vue-3` is now `@tiptap/extension-drag-handle-vue`.
